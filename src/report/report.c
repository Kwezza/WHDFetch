...
void report_write(void)
{
    ...
    for (i = 0; i < pack_count; i++)
    {
        ...
        {
            fprintf(report_file, ", %ld local-cache\n\n", (long)pack_local_cache_reuse);
        }
        else
        {
            fprintf(report_file, "\n\n");
    ...
    fprintf(report_file,
            "Total: %ld new, %ld updated, %ld local-cache reuse (no download).\n",
            (long)total_new,
            (long)total_updated,
    ...
    printf("Report saved: %s (%ld new, %ld updated, %ld local-cache)\n",
           report_path,
           (long)total_new,
           (long)total_updated,
           (long)total_local_cache_reuse);
