...
typedef struct report_entry {
    ...
    char download_failure_reason[REPORT_MAX_FILENAME];
    const char *pack_name;
    BOOL is_update;
    BOOL is_local_cache_reuse;
    BOOL is_extraction_skip;
    BOOL is_download_failure;
} report_entry;
                                 ...
                                 const char *reason)
{
    ...
    strncpy(entry->archive_name, archive_name, REPORT_MAX_FILENAME - 1);
    entry->archive_name[REPORT_MAX_FILENAME - 1] = '\0';
    strncpy(entry->download_failure_reason, reason, REPORT_MAX_FILENAME - 1);
    entry->download_failure_reason[REPORT_MAX_FILENAME - 1] = '\0';
    entry->pack_name = pack_name;
    entry->is_download_failure = TRUE;
...
void report_write(void)
{
    ...
    for (i = 0; i < g_session_report.count; i++)
    {
        ...
        if (g_session_report.entries[i].is_download_failure)
        {
            total_download_failed++;
        }
        else if (g_session_report.entries[i].is_update)
        {
            total_updated++;
        }
    ...
    for (i = 0; i < pack_count; i++)
    {
        ...
        for (j = 0; j < g_session_report.count; j++)
        {
            ...
            if (g_session_report.entries[j].is_download_failure)
            {
                pack_download_failed++;
            }
            else if (g_session_report.entries[j].is_update)
            {
                pack_updated++;
            }
        ...
        if (pack_download_failed > 0)
        {
            fprintf(report_file, ", %ld download-failed", (long)pack_download_failed);
        }

        if (pack_local_cache_reuse > 0 ||
            pack_extraction_skipped > 0 ||
            pack_download_failed > 0)
        {
            fprintf(report_file, "\n\n");
        }
        else
        ...
        if (pack_new > 0)
        {
            ...
            for (j = 0; j < g_session_report.count; j++)
            {
                if (strcmp(g_session_report.entries[j].pack_name, pack_names[i]) == 0 &&
                    ...
                    !g_session_report.entries[j].is_download_failure)
                {
        ...
        if (pack_download_failed > 0)
        {
            fprintf(report_file, "  Download failed:\n");
            for (j = 0; j < g_session_report.count; j++)
            {
                if (strcmp(g_session_report.entries[j].pack_name, pack_names[i]) == 0 &&
                    g_session_report.entries[j].is_download_failure)
                {
                    fprintf(report_file,
                            "    %s  (%s)\n",
                            g_session_report.entries[j].archive_name,
                            g_session_report.entries[j].download_failure_reason);
                }
            }
            fprintf(report_file, "\n");
        }
    ...
    fprintf(report_file,
            "Total: %ld new, %ld updated, %ld local-cache reuse (no download), %ld extraction skipped, %ld download failed.\n",
            (long)total_new,
            (long)total_updated,
            (long)total_local_cache_reuse,
            (long)total_extraction_skipped,
            (long)total_download_failed);

    fclose(report_file);
    amiga_free(pack_names);

        printf("Report saved: %s (%ld new, %ld updated, %ld local-cache, %ld extract-skipped, %ld download-failed)\n",
           report_path,
           (long)total_new,
           (long)total_updated,
           (long)total_local_cache_reuse,
