...
2. Run `whdfetch DOWNLOADGAMES VERBOSE`.
...
- `verbose_output`
  - Test: set `verbose_output=true`.
  - Expected: `config[after-ini]: ... verbose=1 ...` and detailed DAT unzip output.
- `skip_ntsc`
  - Test: set `true`.
...
- Test: set `skip_aga=false` in INI, then run `whdfetch DOWNLOADGAMES SKIPAGA`.
