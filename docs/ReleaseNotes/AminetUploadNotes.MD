...
### Formatting Requirements (from Aminet upload page)

> "Make sure your readme file is formatted correctly: Lines should not be
> longer than 78 characters, a line should be terminated only by a linefeed
> (LF) character (that's the way AmigaOS and Unix do it), *not* with both CR
> (carriage return) and LF (that's the Windows way)."

- **Maximum line length:** 78 characters
- **Line endings:** LF only (`\n`), not CRLF (`\r\n`)
- The `.gitattributes` rule `*.txt text eol=lf` enforces this on commit.
  Always verify the final file before packaging (e.g. `xxd aminet.readme.txt | grep -c 0d` should return 0).
